...
''');
  ...
  group('ProjectDiscovery skip files', () {
    test('hasSkipFile checks tom_skip.yaml first', () {
      final tempDir = Directory.systemTemp.createTempSync('skip_test_');
      try {
        // No skip files
        expect(ProjectDiscovery.hasSkipFile(tempDir.path), false);

        // Add tom_skip.yaml
        File(p.join(tempDir.path, 'tom_skip.yaml')).writeAsStringSync('');
        expect(ProjectDiscovery.hasSkipFile(tempDir.path), true);
        expect(
          ProjectDiscovery.hasSkipFile(tempDir.path, basename: 'testkit'),
          true,
        );
      } finally {
        tempDir.deleteSync(recursive: true);
    ...
    test('hasSkipFile checks tool-specific skip', () {
      final tempDir = Directory.systemTemp.createTempSync('skip_test_');
      try {
        // Add buildkit_skip.yaml
        File(p.join(tempDir.path, 'buildkit_skip.yaml')).writeAsStringSync('');

        // buildkit should be skipped
        expect(ProjectDiscovery.hasSkipFile(tempDir.path), true);
        expect(
          ProjectDiscovery.hasSkipFile(tempDir.path, basename: 'buildkit'),
          true,
        );

        // testkit should NOT be skipped
        expect(
          ProjectDiscovery.hasSkipFile(tempDir.path, basename: 'testkit'),
          false,
        );
    ...
    test('getSkipFileName returns correct file name', () {
      final tempDir = Directory.systemTemp.createTempSync('skip_test_');
      try {
        // No skip files
        expect(ProjectDiscovery.getSkipFileName(tempDir.path), null);

        // Add tom_skip.yaml
        File(p.join(tempDir.path, 'tom_skip.yaml')).writeAsStringSync('');
        expect(
          ProjectDiscovery.getSkipFileName(tempDir.path),
          'tom_skip.yaml',
        );
      } finally {
        tempDir.deleteSync(recursive: true);
  ...
  group('WorkspaceNavigationArgs modes', () {
    test('parseModesOption parses comma-separated modes', () {
      // Test through WorkspaceNavigationArgs since _parseModesOption is private
      final args = WorkspaceNavigationArgs(modes: ['DEV', 'CI']);
      expect(args.modes, ['DEV', 'CI']);
    });

    test('modes are included in copyWith', () {
      final args = WorkspaceNavigationArgs(modes: ['DEV']);
      final copied = args.copyWith(modes: ['PROD']);
      expect(copied.modes, ['PROD']);
    });

    test('modes are included in withDefaults', () {
      final args = WorkspaceNavigationArgs(modes: ['DEV', 'CI']);
      final withDefaults = args.withDefaults();
      expect(withDefaults.modes, ['DEV', 'CI']);
    });
  });
}
