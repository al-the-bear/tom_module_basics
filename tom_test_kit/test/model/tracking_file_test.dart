...
void main() {
  group('TrackingFile', () {
    group('fromBaseline', () {
      test('TK-TRK-1: should create tracking file with entries and one run',
          () {
        final entries = [
          TestEntry(
        ...
        final run = TestRun(
          ...
          isBaseline: true,
          results: {
            'test A': TestResult.ok,
            'test B': TestResult.fail,
          },
        );
    ...
    group('addRun', () {
      test('TK-TRK-2: should append a run and merge new entries', () {
        ...
        final newRun = TestRun(
          ...
          results: {
            'test A': TestResult.ok,
            'new test': TestResult.ok,
          },
        );
        final newEntries = [
          TestEntry(
            fullDescription: 'new test',
    ...
    group('write and load round-trip', () {
      ...
      test('TK-TRK-6: should write and reload a baseline tracking file',
          () async {
        final entries = [
          TestEntry(
            id: 'TK-1',
