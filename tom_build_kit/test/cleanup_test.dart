...
''');
      ...
      final tempFile =
          createTempFile('_build/lib/src/test_temp.g.dart',
...
''');
    ...
    test('protected-folders with multi-segment paths (bug #17 FIXED)',
        () async {
...
''');
      ...
      for (var i = 0; i < 6; i++) {
        createTempFile('_build/lib/src/_test_safe/file_$i.g.dart',
            content: '// Safety test file $i');
      }

      final result = await ws.runTool(
          'cleanup', ['--project', '_build', '--max-files', '3']);
      ...
      for (var i = 0; i < 6; i++) {
        final f = File(p.join(ws.workspaceRoot,
      ...
      final stdout = (result.stdout as String);
      expect(stdout.toLowerCase(), contains('warning'),
...
''');
      ...
      for (var i = 0; i < 6; i++) {
        createTempFile('_build/lib/src/_test_force/file_$i.g.dart',
            content: '// Force test file $i');
      }

      final result = await ws.runTool(
          'cleanup', ['--project', '_build', '--max-files', '3', '--force']);
