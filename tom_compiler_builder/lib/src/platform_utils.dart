...
/// Platform name mappings and conversion utilities.
class PlatformUtils {
  PlatformUtils._();

  /// Get current platform in VS Code format (e.g., darwin_arm64, linux_x64)
  ...
  static bool matchesPlatform(String pattern, String targetPlatform) {
    final normalizedPattern = pattern.toLowerCase().trim().replaceAll('_', '-');
    final normalizedTarget = targetPlatform.toLowerCase().trim().replaceAll('_', '-');
    ...
    if (normalizedPattern.endsWith('*')) {
      final prefix = normalizedPattern.substring(0, normalizedPattern.length - 1);
  ...
  static String getOSFromPlatform(String platform) {
    ...
    if (normalized.startsWith('win32-') || normalized.startsWith('windows-')) {
      return 'windows';
    }
